...
import { getGoogleRefreshToken, getCurrentUser } from "~/lib/firebase";
...
export const loader: LoaderFunction = async ({ request }) => {
  try {
    ...
    if (!refreshToken) {
      return json({ 
        error: `No Google refresh token found for ${currentUser.email}. Please authenticate with Google first.`,
    ...
    return json({ 
      ...
      message: `Successfully fetched users using refresh token for ${currentUser.email}`,
      hasRefreshToken: true,
      tokenValid: true,
      tokenInfo: {
